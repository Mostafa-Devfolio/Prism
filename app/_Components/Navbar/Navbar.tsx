...
import { IProduct5, IVendor5 } from '@/app/interface/search';
...
export default function NavBar() {
  ...
  const currentContext = useMemo(() => {
    if (pathName.includes('/restaurants'))
      return { id: 'restaurants', label: 'Restaurants', icon: <UtensilsCrossed size={14} /> };
    if (pathName.includes('/groceries'))
      return { id: 'groceries', label: 'Groceries', icon: <ShoppingBasket size={14} /> };
    if (pathName.includes('/pharmacies')) return { id: 'pharmacies', label: 'Pharmacies', icon: <Pill size={14} /> };
    if (pathName.includes('/e-commerce')) return { id: 'e-commerce', label: 'E-Commerce', icon: <Store size={14} /> };
    return null;
  }, [pathName]);
  ...
  }, [searchQuery, searchCategory, businessSlug, searchTarget]); // Re-runs if tab changes
  ...
  const menuItems = [
    ...
    { name: 'About', url: '/about', index: 1 },
    { name: 'Contact US', url: '/contact-us', index: 2 },
  ];
  ...
  return (
    <>
      <div className="sticky top-0 z-100 flex w-full flex-col bg-white/95 shadow-sm backdrop-blur-xl sm:hidden">
        <div className="grid grid-cols-3 items-center px-4 py-3">
          <div className="flex items-center gap-2">
            {/* <button
              onClick={() => setIsMenuOpen(!isMenuOpen)}
              className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-slate-100 text-slate-700 transition-transform active:scale-95"
            >
          ...
          <h2 className="truncate px-2 text-center text-xl font-black tracking-tighter text-slate-900 uppercase">
            PRISM
          </h2>
          ...
          <div className="flex shrink-0 justify-end">
            <button
              ...
              className="relative flex h-10 w-10 items-center justify-center rounded-full bg-slate-100 transition-transform active:scale-95"
            >
              <FontAwesomeIcon icon={faShoppingCart} className="h-4 w-4 text-slate-700" />
              {cartDisplayCount > 0 && (
        ...
        {isMenuOpen && (
          <div className="animate-in slide-in-from-top-2 absolute top-full left-0 z-90 w-full space-y-6 border-b border-gray-100 bg-white px-4 py-6 shadow-2xl duration-300">
            <div className="flex flex-col gap-2">
              {menuItems.map((item) => (
                <Link
                  ...
                  className={`w-full rounded-2xl p-4 text-center font-bold transition-colors ${
                    pathName === item.url ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-900 hover:bg-slate-100'
                  }`}
                >
                  {item.name}
                </Link>
      ...
      <nav className="sticky top-0 z-100 hidden w-full border-b border-slate-100 bg-white/85 shadow-sm backdrop-blur-xl transition-all sm:block">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
          <div className="flex min-w-0 items-center gap-4 lg:gap-6">
            <Link
              href="/"
              className="shrink-0 text-2xl font-black tracking-tighter text-slate-900 transition-transform hover:scale-105"
            >
              PRISM
            </Link>
            <div
              onClick={() => router.push('/address')}
              className="flex max-w-35 min-w-0 cursor-pointer items-center gap-2.5 rounded-full border border-slate-200/60 bg-slate-100/80 px-4 py-2 transition-colors hover:bg-slate-200 lg:max-w-60"
            >
              ...
              <div className="flex min-w-0 flex-col justify-center">
                ...
                {!isMounted ? (
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="animate-pulse text-[9px] leading-none font-bold text-blue-600">Locating...</span>
                  </div>
                ) : (
          ...
          <div className="hidden flex-1 items-center justify-center gap-2 lg:flex">
            {menuItems.map((item) => (
              <Link
                key={item.index}
                href={item.url}
                className={`rounded-full px-6 py-2.5 text-sm font-bold transition-all ${pathName === item.url ? 'bg-slate-900 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}`}
              >
                {item.name}
              </Link>
            ))}
          ...
          <div className="flex shrink-0 items-center gap-3">
            <button
              onClick={() => setIsSearchOpen(true)}
              className="flex h-12 w-12 items-center justify-center rounded-full bg-slate-100 text-slate-700 transition-colors hover:bg-slate-200 hover:text-blue-600"
            >
            ...
            {showCart && (
              <button
                onClick={() => router.push('/cart')}
                className="relative flex h-12 w-12 items-center justify-center rounded-full bg-slate-100 text-slate-700 transition-colors hover:bg-slate-200 hover:text-blue-600"
              >
                <ShoppingCart size={20} strokeWidth={2.5} />
                {cartDisplayCount > 0 && (
                  <div className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-red-600 text-[10px] font-black text-white shadow-sm ring-2 ring-white">
                    {cartDisplayCount}
                  </div>
                )}
              </button>
            ...
            <div className="hidden items-center gap-3 lg:flex">
              {!auth ? (
                <>
                  <Link
                    href="/login"
                    className="rounded-full px-6 py-2.5 text-sm font-bold text-slate-700 transition-colors hover:bg-slate-100"
                  >
                    Login
                  </Link>
                  <Link
          ...
          <div className="animate-in slide-in-from-bottom-8 sm:zoom-in-95 flex h-dvh w-full flex-col overflow-hidden bg-white shadow-2xl sm:h-auto sm:max-h-[85vh] sm:max-w-3xl sm:rounded-[2.5rem]">
            ...
            <div className="flex shrink-0 items-center gap-4 border-b border-slate-100 px-4 py-4 sm:px-6 sm:py-6">
              ...
              <input
                autoFocus
                type="text"
                placeholder={currentContext ? `Search in ${currentContext.label}...` : 'Search PRISM...'}
                value={searchQuery}
            ...
            <div className="no-scrollbar flex shrink-0 gap-2 overflow-x-auto border-b border-slate-100 bg-white px-4 py-3 sm:px-6">
              {currentContext ? (
                <div className="flex items-center gap-2 rounded-full bg-slate-900 px-5 py-2 text-sm font-bold text-white">
                  {currentContext.icon} Searching in {currentContext.label}
                </div>
              ) : (
                [
                  { id: 'restaurants', label: 'Restaurants', icon: <UtensilsCrossed size={14} /> },
                  { id: 'groceries', label: 'Groceries', icon: <ShoppingBasket size={14} /> },
                  { id: 'pharmacies', label: 'Pharmacies', icon: <Pill size={14} /> },
                  { id: 'e-commerce', label: 'E-Commerce', icon: <Store size={14} /> },
                ].map((cat) => (
                  <button
                    key={cat.id}
                    onClick={() => {
            ...
            <div className="flex-1 overflow-y-auto bg-slate-50/50 p-4 sm:p-6">
              {!searchQuery ? (
                <div className="mt-12 flex flex-col items-center justify-center gap-4 text-slate-400">
                  ...
                  <p className="text-center font-bold">Type to search for {searchTarget}...</p>
                </div>
              ) : isSearching ? (
                  ...
                  {searchTarget === 'vendors' ? (
                    searchResultsVendors.length > 0 ? (
                      searchResultsVendors.map((vendor: IVendor5) => (
                        ...
                        >
                          {/* <div className="relative h-14 w-14 shrink-0 overflow-hidden rounded-xl border border-slate-100 bg-slate-50">
                            <Image
                              src={vendor.image ? `${baseURL}${vendor.image}` : IMAGE_PLACEHOLDER}
                              fill
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            {/* <p className="mt-0.5 text-[10px] font-bold tracking-widest text-slate-400 uppercase">
                              {currentContext ? currentContext.label : vendor.businessType || 'Store'}
                            </p> */}
                          </div>
                          <ArrowRight
                            size={18}
                      ...
                      <div className="mt-12 flex flex-col items-center justify-center gap-2 text-slate-500 opacity-80">
                        <p className="text-xl font-black text-slate-900">No vendors found</p>
                      </div>
                    )
